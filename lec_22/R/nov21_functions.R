##***************************
##  Software Tools - Functions, debugging, and iteration
##
## Karl Cottenie
##
## 2025-11-20
...
# FIX THIS FUNCTION so it performs the task of counting the number of species for each bin_uri

# Finally, consult "Is ecology ready for big code"
...
# the code from line 86-89 becomes
...
# the function that you use in map does not have to be a function available
...
# you can make the function construction similar to the start of the code
...
# For this week's quiz, create a function that will test Bergmann's rule for a range of definitions of "non-tropical", i.e., by changing the latitude value of 30 in the above code. You will thus create filter that only keeps rows that have an absolute value for latitude greater than cutoff_lat, with cutoff_lat varying from 10 to 50 in steps of 10 degrees.
# You will receive 5 points for creating a function
# Hint: to extract the p-value from a t-test object, you can use this code
# t.test(df_models[, 3], mu = 0)$p.value
# You will receive an additional 5 points for iterating over the different cutoff values
# and finally you will receive an additional 5 points for creating a figure that illustrates whether the significance of Bergmann's rule depends on the definition of "non-tropical".
