...
#ANSWER (to generate and view vector)
my.vector <- 1:20
my.vector

#If you wish, you can also view the vector right away by encasing your command in parentheses.
(my.vector <- 1:20)
...
#ANSWER
my.rnorm <- rnorm(n = 100, mean = 10, sd = 5)
...
#Tip: When sampling or generating random numbers, if you want your script to be completely reproducible, you can use set.seed() first. First, try generating several sets of samples without doing that. Then, try set.seed() and do it again. the parentheses in this example means I am choosing to print the results to the screen right away. Example:
set.seed(12345)
(ponds <- sample(x = 1:200, size = 40))

#Compare if you run it again:
set.seed(12345)
(ponds <- sample(x = 1:200, size = 40))
...
friends <- c("Jen", "Xin", "Joe", "Sal", "Mo", "Bob", "Mia", "Tara", "Ed", "Kim")
sample(x = friends, size = 5)
...
#Example answer: Completing sampling code challenge in one line. This line is sampling thirty elements from a vector of integers 1:100, 40 times, and assigning the results to a new data object, called to.barcode.
to.barcode <- replicate(n = 40, sample(x = 1:100, size = 30))
...
#TIP: Note that we can click on this object in the environment pane, and a viewer will come up in a new tab in the same pane as your script. This is a very helpful feature of RStudio! Does the object look as expected? We have a matrix that is 40 columns wide, representing our 40 ponds. Our matrix has 30 rows, as we now have the specimen number for each individual pond that we are going to sequence. Values range from 1 to 100, as expected.(Note that real data would be more complex! We may have variable numbers of specimens per pond, such as if some sites have low productivity, for example. So, in real life, we would need to sample over the available length of data for each pond... we will be able to do such cases later.)
...
#I will put example answers to challenge #4 in a separate script, so that this script doesn't get overwhelming. Everyone: Feel free to have a look once you are comfortable with this script.
...
#Think: In this case, would we want to sample with or without replacement? Answer: Without replacement! We wouldn't want to sample 2 students twice, and we wouldn't want to leave anyhow out. So, for this purpose, the default (replace = FALSE) is what we want.
...
#This was a brief overview... We will work with dataframes much more and become comfortable with them soon.

###Part 5 - Preparing for next class----
